...
#Linear power flow
P_ev_station_lin = 100e3
P_ev_station_base = 50e3
...
plt.figure(0)
plt.clf()
plt.plot(range(len(net_ieee13.dv_iter)),np.log(net_ieee13.dv_iter),label='dV')
plt.plot(range(len(net_ieee13.dv_iter)),np.log(net_ieee13.di_iter),label='dI')
...
P_ev_station = P_ev_station_lin + 500e3
#net_ieee13.bus_df.loc[bus_index_ev_station,'Pa'] = P_ev_station/3/1e3
#net_ieee13.bus_df.loc[bus_index_ev_station,'Pb'] = P_ev_station/3/1e3
#net_ieee13.bus_df.loc[bus_index_ev_station,'Pc'] = P_ev_station/3/1e3
net_ieee13.bus_df.loc[bus_index_ev_station,'Pa'] = P_ev_station_base/3/1e3
...
b_Pslack = np.real(np.matmul(pf_model.vs.T,np.matmul(np.conj(Ysn),np.matmul(np.conj(M_wye),PQ0_wye))))\
          ...
          +np.real(np.matmul(pf_model.vs.T,(np.matmul(np.conj(Yss),np.conj(pf_model.vs))+np.matmul(np.conj(Ysn),np.conj(pf_model.M0)))))
